  ...
  return (
    <form onSubmit={handleSave} className="max-w-4xl mx-auto bg-(--container) border border-(--wrapper) rounded-2xl p-6 md:p-8 shadow-sm transition-colors mb-12">
      ...
      <div className="mb-6 flex flex-col items-start gap-4">
         <SingleImageField file={imageFile} previewUrl={imagePreview || "/return.png"} onChange={handleImageChange} onRemove={handleImageRemove}/>
      </div>
      ...
      <div className="mb-5 flex items-center gap-2 bg-background p-4 rounded-xl border border-(--wrapper)">
        <input type="checkbox" id="operation_result" name="operation_result" checked={isOperationResult} onChange={(e) => handleInputChange({ target: { name: "operation_result", value: e.target.checked }} as any)} className="w-4 h-4 text-black! dark:text-white! focus:ring-(--header) border-gray-300 rounded cursor-pointer" />
      ...
      <div className="mb-5 flex items-center gap-2 bg-background p-4 rounded-xl border border-(--wrapper)">
        <input type="checkbox" id="human_trafficking_indicators" name="human_trafficking_indicators" checked={isHumanTrafficking} onChange={(e) => handleInputChange({ target: { name: "human_trafficking_indicators", value: e.target.checked }} as any)} className="w-4 h-4 text-black! dark:text-white! focus:ring-(--header) border-gray-300 rounded cursor-pointer" />
      ...
      <div className="flex justify-end gap-3 border-t border-(--wrapper) pt-6 mt-8">
        <button type="button" onClick={onCancel} className="flex items-center gap-1.5 px-4 py-2 bg-stone-200 dark:bg-stone-800 text-slate-800 dark:text-slate-200 font-bold rounded-lg hover:opacity-90 active:scale-[0.98] transition text-sm cursor-pointer">
          <X size={16} /><span>ยกเลิก</span>
        </button>
        <button type="submit" disabled={isSaving} className="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 text-white font-bold rounded-lg hover:opacity-90 active:scale-[0.98] transition text-sm cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
          <Save size={16} /><span>{isSaving ? "กำลังบันทึก..." : "บันทึกข้อมูล"}</span>
        </button>
      </div>
    </form>
  );
