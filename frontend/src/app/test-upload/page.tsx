...
export default function TestUploadPage() {
  ...
  const renderNull = (text = "null") => (
    <span className="text-(--shadow)ic font-normal text-xs">{text}</span>
  );
      ...
      {result && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="p-6 border rounded-xl bg-(--container) shadow-sm border-(--wrapper)">
             ...
             <p className="text-sm mb-4 text-(--header) opacity-80">ตรวจสอบข้อมูลพรีวิวด้านล่าง หากถูกต้องแล้ว กดปุ่มเพื่อนำเข้าฐานข้อมูลจริง</p>
             {isUploading ? (
                      ...
                      <td className="p-4 border-r border-(--wrapper) bg-(--container) opacity-95 align-top text-(--header)">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-3 pb-3 border-b border-(--wrapper)">
                          <div>
                            ...
                            <div className="text-sm">
                              <span className="text-(--header) opacity-50 mr-2">[DB: middle_name_th]</span>
                              <span className="font-medium text-(--blueText)">{row.middle_name_th || renderNull()}</span>
                            </div>
                            <div className="text-sm">
                          ...
                          <div>
                            ...
                            <div className="text-sm">
                              <span className="text-(--header) opacity-50 mr-2">[DB: middle_name_en]</span>
                              <span className="font-medium text-(--blueText)">{row.middle_name_en || renderNull()}</span>
                            </div>
                            <div className="text-sm">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-3 pb-3 border-b border-(--wrapper)">
                          <div>
                            <span className="text-(--header) opacity-60 text-xs block">[DB: nationality] สัญชาติ:</span>
                            <span className="font-medium">{row.nationality || renderNull()}</span>
                          </div>
                          <div>
                            <span className="text-(--header) opacity-60 text-xs block">[DB: passport_id] พาสปอร์ต:</span>
                            <span className="font-mono font-medium text-(--blueText)">{row.passport_id || renderNull()}</span>
                          </div>
                          <div>
                            <span className="text-(--header) opacity-60 text-xs block">[DB: gender] เพศ (อัตโนมัติ):</span>
                          ...
                          <div>
                            ...
                            <span className="font-medium bg-(--yellowBG) px-1.5 rounded text-(--yellowText) border border-(--yellowBorder)">
                        ...
                        <div className="bg- (--container)] rounded-lg p-3 border border- (--wrapper)]">
                          <span className="text-[10px] font-semibold text- (--header)] opacity-50 uppercase tracking-wider block mb-2">หมวดหมู่ผลการคัดกรอง</span>
                          ...
                          <div>
                            <span className="text- (--blueText)] text-xs font-semibold block mb-1">
                              [DB: screening_details] รายละเอียดผลคัดกรอง:
                            </span>
                            <div className="text-sm font-medium text- (--header)] bg- (--button)] p-2 border border- (--wrapper)] rounded-md min-h-10 whitespace-pre-wrap">
